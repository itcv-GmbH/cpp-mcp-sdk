...
//
// Note: Runners live in namespace mcp (not mcp::server) to align with the existing
// public API surface where core types like Server, Client, and Session reside.
//
...
using StdioServerRunner = server::StdioServerRunner;
using StreamableHttpServerRunner = server::StreamableHttpServerRunner;
using CombinedServerRunner = server::CombinedServerRunner;
