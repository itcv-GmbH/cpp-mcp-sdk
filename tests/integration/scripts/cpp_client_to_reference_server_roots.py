#!/usr/bin/env python3
"""Run C++ client roots fixture against Python reference server."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run C++ SDK client roots fixture against reference Python server"
    )
    parser.add_argument(
        "--cpp-client",
        required=True,
        help="Path to C++ client roots fixture executable",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    exit_code = run()
    print(
        json.dumps({"result": "ok", "script": "cpp_client_to_reference_server_roots"})
    )
