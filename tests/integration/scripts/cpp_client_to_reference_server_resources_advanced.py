#!/usr/bin/env python3
"""Run C++ client resources advanced fixture against Python reference server."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run C++ SDK client resources advanced fixture against reference Python server"
    )
    parser.add_argument(
        "--cpp-client",
        required=True,
        help="Path to C++ client resources advanced fixture executable",
    )
    parser.add_argument(
...
def run() -> int:
    ...
    try:
        ...
        if exit_code != 0:
            raise RuntimeError(
                "C++ client resources advanced fixture failed.\n"
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            {
                "result": "ok",
                "script": "cpp_client_to_reference_server_resources_advanced",
            }
        )
    )
