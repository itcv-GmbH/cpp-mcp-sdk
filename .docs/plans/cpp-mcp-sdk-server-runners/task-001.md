...
1. Runners must live in `namespace mcp` to align with the existing public API surface; this decision is required to be documented in header comments.
...
3. Define the minimal surface for Streamable HTTP:
     ...
     - documentation is required to state that `StreamableHttpServerOptions.http.requireSessionId = true` is the multi-client-safe configuration
4. Ensure API design explicitly addresses Streamable HTTP session isolation (delegated to `task-002`).
5. Define the minimal surface for the combined runner:
   - a single class that will support enabling STDIO, HTTP, or both via explicit configuration flags
